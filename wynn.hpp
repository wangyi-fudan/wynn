...
struct	global{
	...
	float	learning_rate;
	global(){	set_threads(omp_get_num_procs()>1?omp_get_num_procs()-1:1);	prng=time(NULL);	learning_rate=1.0f/(1ull<<16);	}
	void	set_threads(uint64_t	n){	threads=n;	omp_set_num_threads(threads);	setNbThreads(threads);	}
}global;
